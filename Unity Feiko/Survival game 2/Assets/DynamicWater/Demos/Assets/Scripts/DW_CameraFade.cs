...
public class DW_CameraFade : MonoBehaviour {
    private static DW_CameraFade mInstance;

    private static DW_CameraFade instance {
        get {
            if (mInstance == null) {
                mInstance = FindObjectOfType(typeof (DW_CameraFade)) as DW_CameraFade ??
                            new GameObject("DW_CameraFade").AddComponent<DW_CameraFade>();
            }
